...
#define debug_mode 1
#define overwrite_fallback false //true: overwrite this wifi in the memory. false: use this wifi as a fallback wifi when the configured wifi is not available
ESP8266webadmin ESPConfig("myESP", 80, ESP_config_reset_pin, overwrite_fallback, "fallback_wifi", "fallback_pass", debug_mode);
  ...
  if ((ESPConfig.wifi_connected) && (ESPConfig.ESP_mode == CLIENT_MODE))
  {
    if (((millis_now - conn_timer) > heartbeat) || (conn_timer == 0) )
    {
      ...
      if (node_token.length() == 24)
      {
		...
		if(n_vars>=0)  //check if variables sync request was successful
		{
			...
			if (first_sync) first_sync = false;
